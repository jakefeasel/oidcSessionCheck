...
```javascript
    var sessionCheck = new SessionCheck({
        ...
        // optional. Only called when using `response_type=id_token`
        ...
        // optional - only used with `response_type=id_token`
        scope: "openid"
    });
```
...
*Details you need to provide:*
 ...
 - sessionClaimsHandler [optional] - function to be called after every successful session check, with latest claims included. Only used with `response_type=id_token`.
